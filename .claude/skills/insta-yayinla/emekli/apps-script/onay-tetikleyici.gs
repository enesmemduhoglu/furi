/**
 * ===========================================================================
 *  EMEKLI — BU SCRIPT ARTIK KULLANILMIYOR  (2026-08-16, SaaS entegrasyonu)
 * ===========================================================================
 *
 *  Onay ve yayin content-approval-saas'a tasindi: onay geldigi an yayin ayni
 *  HTTP isteginde yapiliyor (olculen ~11 saniye). Gmail'i izleyip Claude
 *  rutinini tetikleyen bu zincire artik gerek yok.
 *
 *  Dosya SILINMEDI — geri donus yolu acik kalsin diye emekli olarak duruyor.
 *  Neden emekli oldugu, neyin yerine gectigi ve nasil geri alinacagi:
 *  ../README.md
 *
 *  Kod olarak da DEVRE DISI: asagidaki EMEKLI bayragi true oldugu surece
 *  onayKontrol() hicbir sey yapmaz. Kazayla deploy edilse bile Gmail'i
 *  taramaz, GitHub'a yorum atmaz. Geri almak tek satir: EMEKLI = false.
 *
 * ---------------------------------------------------------------------------
 *  FURI — Instagram yayin onayi tetikleyicisi  (ozgun aciklama)
 *
 *  Gmail'de [FURI-ONAY] konulu maillere gelen "EVET" / "HAYIR" yanitlarini
 *  yakalar ve Claude rutinini tetikler. Google'in sunucusunda dakikada bir
 *  calisir; Claude tarafinda hicbir bosa calisma olmaz.
 *
...
/**
 * true  -> script inert (bugunku durum)
 * false -> eski Gmail/GitHub tetikleme zinciri yeniden calisir
 */
var EMEKLI = true;

/** Emekliyse logla ve true don; cagiran fonksiyon oldugu yerde durur. */
function emekliMi_() {
  if (!EMEKLI) return false;
  Logger.log(
    'onay-tetikleyici EMEKLI: yayini content-approval-saas yapiyor, bu script ' +
    'hicbir sey yapmadi. Geri almak icin ../README.md'
  );
  return true;
}
